...
void PlayerAmmoBar::UpdateUI() 
{
	...
	for (Weapon* w : allWeapons)
	{
		...
		{ 
			...
			{
				Debug::DrawCircle(centerCam.x - 610 + i * 45, centerCam.y - 245, 15, sf::Color(0, 150, 255)); 
			} 

			for (int i = currentAmmos; i < w->GetMaxAmmos(); ++i)
			{
				Debug::DrawCircle(centerCam.x - 610 + i * 45, centerCam.y - 245, 15, sf::Color(0, 0, 75));
			}
		...
		{
			Debug::DrawFilledRectangle(playerPos.x - playerSize.x * 0.25f, playerPos.y - playerSize.y * 0.5f - 20, { 50, 12 }, sf::Color(150, 0, 0, 255)); 
			Debug::DrawFilledRectangle(playerPos.x - playerSize.x * 0.25f, playerPos.y - playerSize.y * 0.5f - 20, { 50 * ratio, 12 }, sf::Color::Green); 
